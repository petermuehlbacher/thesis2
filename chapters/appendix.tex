...
for the the case where $A$ is deterministic and positive definite. One can extend this result to arbitrary matrices of operator norm $O(1)$ by noting that it holds for Hermitian matrices $M=M^*=M_+-M_-$ ($M_+\geq 0,M_-\geq 0,\|M\|_{OP}=O(1)$) by applying the triangle inequality and %for general matrices $M$ of of operator norm $\|M\|_{op}=O(1)$ by using their polar decomposition $M=UP$, $U$ unitary and $P\geq 0$ being unique if $M$ is invertible to get the bound $|X^*MX|=|X^*UPX|\leq||$
\todo{polar decomposition doesn't seem to work, for diagonalisable matrices it's clear and the probability of coinciding eigenvalues should go to $0$, so maybe that suffices(?)}
